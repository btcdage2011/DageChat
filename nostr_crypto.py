...
class BIP340:
    ...
    def sign(priv_key_bytes, msg_bytes, aux_rand):
        ...
        d = d0 if P[1] % 2 == 0 else BIP340.n - d0
        t = (d ^ BIP340.int_from_bytes(BIP340.tagged_hash('BIP0340/aux', aux_rand))).to_bytes(32, 'big')
        ...
        k = k if R[1] % 2 == 0 else BIP340.n - k
        ...
        sig = BIP340.bytes_from_int(R[0]) + BIP340.bytes_from_int((k + e * d) % BIP340.n)
        return sig
...
class NostrCrypto:
    ...
    def generate_private_key_hex():
        while True:
            ...
            if priv.public_key.format(compressed=True)[0] == 2:
                return priv.secret.hex()
        ...
        try:
            ...
            try:
                ...
                if hasattr(priv, 'schnorr_sign'):
                    sig = priv.schnorr_sign(msg_bytes, aux_bytes)
                    return sig.hex()
            except:
                pass
    ...
    def _get_conversation_key(priv_hex, pub_hex):
        try:
            ...
            return hkdf.derive(shared_point)
        except:
            return None
