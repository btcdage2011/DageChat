...
class LoginWindow(SafeToplevel):
    ...
    def login_existing(self):
        ...
        if nick == '(暂无账号)' or nick == tr('LOGIN_NO_ACCOUNT') or (not nick):
            return
        if not pwd:
            messagebox.showwarning(tr('DIALOG_INFO_TITLE'), tr('LOGIN_MSG_ENTER_PWD'))
            return
    ...
    def create_new(self):
        ...
        if not nick:
            return messagebox.showwarning(tr('DIALOG_INFO_TITLE'), tr('LOGIN_MSG_ENTER_NICK'))
        if not p1:
            return messagebox.showwarning(tr('DIALOG_INFO_TITLE'), tr('LOGIN_MSG_ENTER_PWD'))
        if len(p1) < 8:
            return messagebox.showwarning(tr('DIALOG_WARN_TITLE'), tr('LOGIN_MSG_PWD_LEN'))
        if p1 != p2:
            return messagebox.showerror(tr('DIALOG_ERROR_TITLE'), tr('LOGIN_MSG_PWD_MISMATCH'))
        folder_path = os.path.join(self.data_root, f'user_{nick}')
        if os.path.exists(folder_path):
            return messagebox.showerror(tr('DIALOG_ERROR_TITLE'), tr('LOGIN_MSG_USER_EXISTS'))
        try:
            ...
            messagebox.showerror(tr('DIALOG_ERROR_TITLE'), tr('LOGIN_MSG_DIR_ERR').format(e=e))
    ...
    def import_login(self):
        ...
        if not folder_name or not hex_key or (not pwd):
            return messagebox.showwarning(tr('DIALOG_ERROR_TITLE'), tr('LOGIN_MSG_INFO_INCOMPLETE'))
        if len(pwd) < 8:
            return messagebox.showwarning(tr('DIALOG_WARN_TITLE'), tr('LOGIN_MSG_PWD_LEN'))
        if pwd != pwd2:
            return messagebox.showerror(tr('DIALOG_ERROR_TITLE'), tr('LOGIN_MSG_PWD_MISMATCH'))
        ...
        try:
            ...
            messagebox.showerror(tr('DIALOG_ERROR_TITLE'), tr('LOGIN_MSG_IMPORT_ERR').format(e=e))
...
class LocalBlockListWindow(SafeToplevel):
    ...
    def load_list(self):
        ...
        if not blocked:
            ctk.CTkLabel(self.scroll, text=tr('BLOCK_WIN_NONE'), text_color='gray').pack(pady=50)
            return
        for u in blocked:
            pk, name = (u['pubkey'], u['name'] or tr('MSG_SENDER_UNKNOWN'))
            row = ctk.CTkFrame(self.scroll, fg_color='transparent')
            row.pack(fill='x', pady=2)
            ctk.CTkLabel(row, text=f'🚫 {name} ({pk[:6]}...)', width=200, anchor='w').pack(side='left', padx=5)
            ctk.CTkButton(row, text=tr('BLOCK_BTN_LIFT'), width=80, fg_color='green', command=lambda p=pk: self.unblock(p)).pack(side='right', padx=5)
...
class SelectContactDialog(SafeToplevel):
    ...
    def __init__(self, contacts, on_select_callback):
        super().__init__()
        self.title('选择好友')
...
class SelectSessionDialog(SafeToplevel):
    ...
    def __init__(self, parent_app, on_select_callback):
        ...
        for gid, info in groups.items():
            ...
            icon = '🎭' if gtype == 1 else '📢'
            color = '#FF79C6' if gtype == 1 else '#69F0AE'
...
class ExportSelectionDialog(SafeToplevel):
    ...
    def __init__(self, parent_app, target_id=None, target_name='All'):
        ...
        ctk.CTkButton(quick_frame, text='全部', width=60, height=24, fg_color='#555', command=lambda: self.set_quick_date(0)).pack(side='left', padx=5)
        ctk.CTkButton(quick_frame, text='近7天', width=60, height=24, fg_color='#555', command=lambda: self.set_quick_date(7)).pack(side='left', padx=5)
        ctk.CTkButton(quick_frame, text='近30天', width=60, height=24, fg_color='#555', command=lambda: self.set_quick_date(30)).pack(side='left', padx=5)
        ctk.CTkButton(quick_frame, text='今年', width=60, height=24, fg_color='#555', command=lambda: self.set_quick_date(365)).pack(side='left', padx=5)
        btn_frame = ctk.CTkFrame(self, fg_color='transparent')
    ...
    def set_quick_date(self, days):
        ...
        if days == 0:
            return
        now = datetime.now()
        self.entry_end.insert(0, now.strftime('%Y-%m-%d'))
        if days == 365:
    ...
    def start_export(self):
        ...
        if s_ts == -1 or e_ts == -1:
            self.show_toast('❌ 日期格式错误 (应为 YYYY-MM-DD)')
            return
...
class EmojiWindow(SafeToplevel):
    ...
    def show_at(self, target_x, target_y):
        screen_h = self.winfo_screenheight()
        screen_w = self.winfo_screenwidth()
        final_x = target_x
        final_y = target_y - self.height - 10
        if final_y < 0:
            final_y = 0
        if final_x + self.width > screen_w:
            final_x = screen_w - self.width
        self.geometry(f'{self.width}x{self.height}+{final_x}+{final_y}')
        self.deiconify()
        self.attributes('-topmost', True)
        self.focus_force()
