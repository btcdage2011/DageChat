...
class ChatApp(ctk.CTk):
    ...
    def prepare_combined_forward(self):
        ...
        if total_size > 800 * 1024:
            messagebox.showwarning(tr('WARN_SIZE_LIMIT'), tr('WARN_SIZE_MSG').format(s=total_size // 1024))
            return
                ...
                try:
                    for m in msgs:
                        ...
                        if is_me:
                            ...
                            if not my_name:
                                my_name = f'User {get_npub_abbr(self.client.pk)}'
                            sender_name = my_name
                        else:
                            sender_name = self.client.db.get_contact_name(sender_pk) or 'User'
                        item = {'n': sender_name, 't': ts, 'c': raw_content}
                        if raw_content.strip().startswith('{'):
                            try:
                                ...
                                if d.get('image'):
                                    ...
                                    if compressed_b64:
                                        item['i'] = compressed_b64
                                    else:
                                        item['c'] += tr('EXP_IMG_IGNORE')
                            except:
                                pass
                        history_payload['items'].append(item)
                    final_json = json.dumps(history_payload, ensure_ascii=False)
                    if len(final_json.encode('utf-8')) > 700 * 1024:
                        self.show_toast(tr('TOAST_PKG_TOO_LARGE'))
                        return
                    if target_type == 'group':
                        self.client.send_group_msg(target_id, final_json)
                    elif target_type == 'dm':
                        enc = self.client.db.get_contact_enc_key(target_id)
                        if enc:
                            self.client.send_dm(target_id, final_json, enc)
    ...
    def open_emoji_panel(self):
        ...
        try:
            root_x = self.tool_bar.winfo_rootx()
            root_y = self.tool_bar.winfo_rooty()
            self.emoji_window.show_at(root_x, root_y)
        except Exception as e:
    ...
    def start_backend_secure(self, db_file, mode, password, nickname=None, priv_key=None):
        try:
            ...
            if not new_lock.acquire():
                messagebox.showerror('启动失败', '该账号已在运行中！\n\n为了保护数据安全，禁止多开同一账号。\n请检查是否已打开了窗口，或在任务管理器中结束旧进程。')
                return
    ...
    def add_message_bubble(self, msg_id, content, is_me, sender_name, time_ts, reply_to_id=None, sender_pk=None, scroll_to_bottom=True, insert_at_top=False, top_anchor=None):
        ...
        if is_me:
            ...
            bubble_color = '#D1C4E9' if is_ghost_grp else '#FFFFFF'
        ...
        if is_history_card and parsed_data:
            ...
            if scroll_to_bottom:
                ...
                def _do_scroll():
                    try:
                        target_frame._parent_canvas.yview_moveto(1.0)
                    except:
                        pass
                self.after(50, _do_scroll)
                self.after(200, _do_scroll)
            return bubble_frame
        if reply_to_id:
            ...
            if orig_msg:
                ...
                if orig_msg[5]:
                    ...
                    try:
                        o_sender_name = json.loads(orig_msg[3]).get('alias', 'Anon')
                    except:
                        pass
                else:
                    n = self.client.db.get_contact_name(o_sender_pk)
                    if n:
                        o_sender_name = n
                raw_c = orig_msg[3]
                disp_c = raw_c
                if raw_c.strip().startswith('{'):
                    try:
                        ...
                        if js.get('image'):
                            try:
                                img_data = base64.b64decode(js['image'])
                                pil_img = Image.open(BytesIO(img_data))
                                pil_img.thumbnail((120, 120))
                                q_thumb = ctk.CTkImage(light_image=pil_img, dark_image=pil_img, size=pil_img.size)
                            except:
                                pass
                    except:
                        pass
                disp_c = disp_c.replace('\n', ' ')
                limit = 300
                if len(disp_c) > limit:
                    disp_c = disp_c[:limit] + '...'
                q_text = f'{o_sender_name}: {disp_c}'
            if q_thumb:
                thumb_lbl = ctk.CTkLabel(quote_frame, text='', image=q_thumb)
                thumb_lbl.image = q_thumb
                thumb_lbl.pack(side='left', padx=(5, 0), pady=2)
                thumb_lbl.bind('<Button-3>', lambda e, rid=reply_to_id: self.show_quote_context_menu(e, rid))
        ...
        if text_display:
            ...
            calc_height = lines * 24 + 15
            height = min(calc_height, 500)
            show_scrollbar = calc_height > 500
            msg_box = ctk.CTkTextbox(container, width=300, height=height, fg_color='transparent', text_color='black', font=('Microsoft YaHei UI', 14), activate_scrollbars=show_scrollbar)
    ...
    def _render_history_card(self, container, data, msg_id, is_me, sender_pk):
        ...
        container.configure(width=280)
        title_lbl = ctk.CTkLabel(container, text=title, font=('Microsoft YaHei UI', 13, 'bold'), text_color='black' if not is_me else '#222')
        ...
        for i, item in enumerate(items[:4]):
            ...
            if len(content) > 15:
                content = content[:15] + '...'
            row_text = f'{name}: {content}'
            ctk.CTkLabel(summary_frame, text=row_text, anchor='w', font=('Microsoft YaHei UI', 11), text_color='#555').pack(fill='x')
        if len(items) > 4:
    ...
    def run_export_task(self, target_id, file_path, fmt, start_ts=0, end_ts=0):
        try:
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                ...
                if fmt == 'html':
                    f.write(f"\n                    <html><head><meta charset='utf-8'><style>\n                    body {{ font-family: sans-serif; background: #f0f0f0; padding: 20px; }}\n                    .session-header {{ background: #ddd; padding: 10px; margin-top: 30px; border-radius: 5px; font-weight: bold; color: #333; }}\n                    .msg {{ margin-bottom: 10px; padding: 10px; border-radius: 8px; max-width: 80%; clear: both; position: relative;}}\n                    .msg.me {{ background: #95ec69; float: right; }}\n                    .msg.other {{ background: #ffffff; float: left; }}\n                    .meta {{ font-size: 10px; color: #888; margin-bottom: 4px; }}\n                    .content {{ font-size: 14px; word-wrap: break-word; }}\n                    img {{ max-width: 300px; max-height: 300px; border-radius: 4px; display: block; margin-top: 5px; }}\n                    .clear {{ clear: both; }}\n                    </style></head><body>\n                    <h1>{tr('EXP_HTML_TITLE')}</h1>\n                    <p>{tr('EXP_TIME_PREFIX')} {now_str}</p>\n                    ")
                else:
                    f.write(tr('EXP_TXT_HEADER').format(time=now_str))
                    f.write('=' * 50 + '\n')
                for m in msgs:
                    gid, gname, ts, sname, content, is_me, spk, stype_raw = m
                    if is_me:
                        final_sender_name = f"{tr('EXP_SENDER_ME')} ({get_npub_abbr(self.client.pk)})"
                    else:
                        pure_name = sname.split(' (')[0]
                        if pure_name.startswith('USER ') or pure_name == 'USER':
                            pure_name = tr('EXP_SENDER_USER')
                        elif pure_name.startswith('用户 ') or pure_name == '用户':
                            pure_name = tr('EXP_SENDER_USER')
                        final_sender_name = f'{pure_name} ({get_npub_abbr(spk)})'
                    if gid != current_session_id:
                        current_session_id = gid
                        type_tag_map = {'dm': tr('EXP_TYPE_DM'), 'ghost': tr('EXP_TYPE_GHOST'), 'group': tr('EXP_TYPE_NORMAL')}
                        if gid == OFFICIAL_GROUP_CONFIG['id']:
                            type_tag = tr('EXP_TYPE_OFFICIAL')
                        else:
                            type_tag = type_tag_map.get(stype_raw, stype_raw)
                        display_id = gid
                        if stype_raw == 'dm':
                            display_id = to_npub(gid)
                        elif stype_raw == 'ghost':
                            display_id = get_npub_abbr(gid)
                        header_str = tr('EXP_SESSION_HEADER').format(type=type_tag, name=gname, id=display_id)
                        if fmt == 'html':
                            f.write(f"<div class='clear'></div><div class='session-header'>{header_str}</div>")
                        else:
                            f.write(f'\n\n{header_str}\n' + '-' * 50 + '\n')
                    text_part = content
                    img_html = ''
                    if content.startswith('{'):
                        try:
                            ...
                            if fmt == 'html' and data.get('image'):
                                b64 = data['image']
                                img_html = f'<img src="data:image/jpeg;base64,{b64}">'
                            elif fmt == 'txt' and data.get('image'):
                                text_part += tr('EXP_IMG_IGNORE')
                        except:
                            pass
                    time_str = datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                    if fmt == 'html':
                        cls = 'me' if is_me else 'other'
                        safe_text = text_part.replace('<', '&lt;').replace('>', '&gt;')
                        f.write(f"\n                        <div class='msg {cls}'>\n                            <div class='meta'>{final_sender_name} - {time_str}</div>\n                            <div class='content'>{safe_text}{img_html}</div>\n                        </div>\n                        ")
                    else:
                        f.write(f'[{time_str}] {final_sender_name}: {text_part}\n')
                if fmt == 'html':
                    f.write("<div class='clear'></div></body></html>")
            self.after(0, lambda: messagebox.showinfo('Success', f'Saved to {file_path}'))
        except Exception as e:
            self.after(0, lambda: messagebox.showerror(tr('DIALOG_ERROR_TITLE'), str(e)))
    ...
    def rename_group_local(self, group_id=None):
        ...
        if not target_gid:
            return
        new_name = ctk.CTkInputDialog(text='New Name:', title=tr('INFO_BTN_RENAME')).get_input()
    ...
    def process_invite(self, pubkey, manual=False):
        ...
        if manual:
            raw_inp = ctk.CTkInputDialog(text='Pubkey (Hex / npub1):', title=tr('BTN_MANUAL_INPUT')).get_input()
            if not raw_inp:
                self.invite_window = None
                return
    ...
    def create_group_dialog(self):
        ...
        def _update_selected_members(selected_pks):
            ...
            lbl_selected_count.configure(text=f'Selected {count}' if count > 0 else 'None', text_color='#69F0AE' if count > 0 else 'gray')
        ...
        def _do_join():
            ...
            if inp.startswith('dage://ghost/'):
                try:
                    ...
                    messagebox.showerror(tr('DIALOG_ERROR_TITLE'), f'Code Error: {e}')
                    return
            if '|' not in raw_invite_str:
                messagebox.showerror(tr('DIALOG_ERROR_TITLE'), 'Invalid Format')
                return
            try:
                parts = [p.strip() for p in raw_invite_str.split('|')]
                if len(parts) < 6:
                    return messagebox.showerror('Error', 'Invalid Code')
                ...
                if provided_sum != calculated_sum:
                    return messagebox.showerror(tr('DIALOG_WARN_TITLE'), 'Checksum Failed')
                ...
                if g_type == 1:
                    ...
                    from nacl.secret import SecretBox
                    self.client.groups[gid] = {'box': SecretBox(bytes.fromhex(key)), 'name': group_name, 'key_hex': key, 'type': 0}
                if owner:
                ...
                messagebox.showerror(tr('DIALOG_ERROR_TITLE'), f'Join Error: {e}')
        ctk.CTkButton(tab_join, text=tr('BTN_JOIN_NOW'), command=_do_join, fg_color='#1F6AA5').pack(pady=20)

    def add_contact_dialog(self):
        raw_pk = ctk.CTkInputDialog(text='Pubkey (Hex / npub1):', title=tr('BTN_NEW_CONTACT')).get_input()
        if not raw_pk:
            return
        clean_pk = to_hex_pubkey(raw_pk)
        if clean_pk and len(clean_pk) == 64:
            if clean_pk == self.client.pk:
                messagebox.showerror(tr('DIALOG_ERROR_TITLE'), 'Cannot add yourself!')
                return
            ...
            messagebox.showerror(tr('DIALOG_ERROR_TITLE'), 'Invalid Pubkey')
