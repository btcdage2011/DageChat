...
class AsyncRelayWorker:
    ...
    async def _handle_message(self, data):
        if self.latency == -1 and self.ping_start > 0:
    ...
    def send(self, message):
        ...
        if self.manager.loop:
            try:
                asyncio.run_coroutine_threadsafe(self.send_str(message), self.manager.loop)
                return True
            except Exception as e:
                print(f'❌ 发送调度失败: {e}')
                return False
...
class AsyncRelayManager:
    ...
    def get_status_snapshot(self):
        details = []
        connected_count = 0
        current_items = list(self.workers.items())
        for url, w in current_items:
            if w.status == 2:
                connected_count += 1
...
class PersistentChatUser:
    ...
    def _do_subscriptions_for_worker(self, worker):
        ...
        def _send(msg):
            try:
                ...
                loop.create_task(worker.send_str(msg))
            except RuntimeError:
        ...
        _send(json.dumps(req_payload))
        last_db_ts = self.db.get_last_broadcast_time()
        now = int(time.time())
        if now - last_db_ts > 86400:
            print(f'⏰ [System] 距离上次广播已超过 1 天 (或新设备)，执行维护性广播...')
            ...
            hours_left = 24 - int((now - last_db_ts) / 3600)
            print(f'✅ [System] 资料已是最新 (上次广播: {hours_left} 小时前)，本次跳过自动广播。')
        print(f'✅ [Net] 订阅完成 ({worker.url})')
        ...
        try:
            ...
            if msg_type == 'EVENT':
                ...
                if self.db.event_exists(eid):
                    return
                if kind not in [4, 1059] and self.db.is_blocked(event['pubkey']):
                    return
                if kind == 1059:
                    rumor_dict = None
                    ...
                    if recipient == self.pk:
                        try:
                            sender_pk, rumor_dict = NostrCrypto.unwrap_gift(self.priv_k, event)
                        except:
                            pass
                        ...
                        if str(grp.get('type')) == '1':
                            try:
                                ...
                                _, g_rumor = NostrCrypto.unwrap_gift(grp_priv, event)
                                if g_rumor:
                                    rumor_dict = g_rumor
                ...
                if len(data) >= 3:
                    eid = data[1]
                    success = data[2]
                    info = data[3] if len(data) > 3 else ''
                    if not success:
                        print(f'❌ [Relay] 消息被拒: {info}')
            elif msg_type == 'EOSE':
                sub_id = data[1] if len(data) > 1 else '?'
                print(f'✅ [Relay] 历史同步完成 ({sub_id})')
            elif msg_type == 'NOTICE':
                print(f'⚠️ [Relay Notice] {data[1]}')
        except Exception as e:
    ...
    def _handle_dm(self, event):
        ...
        nk, k = (payload.get('name'), payload.get('k'))
    ...
    def _process_ghost_payload(self, event, payload, sender_pk, origin_event_pubkey):
        try:
            ...
            print(f'❌ [Debug] Process Ghost Payload Error: {e}')
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            if not is_ghost and msg_type in ['ban', 'unban']:
                ...
                if event['pubkey'] == owner:
                    ...
                    nm = self.db.get_contact_name(tpk)
                    disp = f'{nm} ({tpk[:6]}...)' if nm else f'User ({tpk[:6]}...)'
                    if msg_type == 'ban':
                        self.db.add_group_ban(gid, tpk, payload.get('text'))
                        sys_msg = f"🛡️ {tr('MENU_OWNER_BAN')} target: {disp}"
                        self.db.save_message(event['id'], gid, owner, sys_msg, event['created_at'], False)
                        self._print_to_ui('system_center', {'group_id': gid, 'text': sys_msg, 'time': event['created_at']})
                    else:
                        self.db.remove_group_ban(gid, tpk)
                        sys_msg = f"📢 {tr('TOAST_UNBLOCK_USER')} target: {disp}"
                        self.db.save_message(event['id'], gid, owner, sys_msg, event['created_at'], False)
                        self._print_to_ui('system_center', {'group_id': gid, 'text': sys_msg, 'time': event['created_at']})
                return
            ...
            if self.db.get_group_owner(gid) == self.pk:
                if event['pubkey'] not in self.db.get_group_members(gid):
                    self.db.add_group_member(gid, event['pubkey'])
    ...
    def _send_event_to_worker(self, worker, kind, content, tags=[]):
        ...
        if hasattr(worker, 'send_str'):
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(worker.send_str(msg_str))
            except RuntimeError:
                if self.relay_manager.loop:
    ...
    def send_dm(self, target_pk, text, target_enc_unused=None, reply_to_id=None, image_base64=None):
        ...
        try:
            ...
            print(f'❌ [Debug] make_gift_wrap 崩溃: {e}')
            return None
        if not wrap_event:
            return None
    ...
    def send_ghost_msg(self, gid, text, image_base64=None, reply_to_id=None):
        ...
        try:
            ...
            data = {'text': text, 'alias': self.db.get_contact_name(self.pk) or 'Anon', 'nonce': nonce, 'ts': int(time.time())}
            if image_base64:
    ...
    def send_group_msg(self, gid, text, reply_to_id=None, image_base64=None, mention_pks=None):
        ...
        if gid == OFFICIAL_GROUP_CONFIG['id']:
            target_diff = OFFICIAL_GROUP_POW_DIFFICULTY
        else:
            target_diff = NORMAL_GROUP_POW_DIFFICULTY
        mined_evt = None
        if target_diff > 0:
