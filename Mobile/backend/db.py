...
class DageDB:
    ...
    def update_my_profile(self, profile_dict):
        with self.lock:
            cursor = self.conn.cursor()
            try:
                cursor.execute('SELECT name, picture, about, website, ln FROM account LIMIT 1')
                row = cursor.fetchone()
                curr_name = row[0] if row else ''
                curr_pic = row[1] if row else ''
                curr_about = row[2] if row else ''
                curr_web = row[3] if row else ''
                curr_ln = row[4] if row else ''
                new_name = profile_dict.get('name')
                if new_name and new_name.strip():
                    curr_name = new_name
                new_pic = profile_dict.get('picture')
                if new_pic and new_pic.strip():
                    curr_pic = new_pic
                new_about = profile_dict.get('about')
                if new_about is not None:
                    if new_about.strip():
                        curr_about = new_about
                new_web = profile_dict.get('website')
                if new_web and new_web.strip():
                    curr_web = new_web
                new_ln = profile_dict.get('ln')
                if new_ln and new_ln.strip():
                    curr_ln = new_ln
                cursor.execute('\n                    UPDATE account SET name=?, picture=?, about=?, website=?, ln=?\n                ', (curr_name, curr_pic, curr_about, curr_web, curr_ln))
                self.conn.commit()
            finally:
                cursor.close()
